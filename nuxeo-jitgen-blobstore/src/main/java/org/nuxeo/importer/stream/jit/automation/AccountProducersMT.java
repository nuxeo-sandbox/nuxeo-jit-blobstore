...
import org.nuxeo.importer.stream.jit.AccountMessageProducerFactoryMT;
...
@Operation(id = AccountProducersMT.ID, category = Constants.CAT_SERVICES, label = "Produces Bank Statements", since = "11.1", description = "")
public class AccountProducersMT {
	private static final Log log = LogFactory.getLog(AccountProducersMT.class);

	public static final String ID = "StreamImporter.runAccountProducersMT";
	...
	public void run(Blob csvData) throws OperationException {
		...
		if (splitOutput) {
			manager.createIfNotExists(MultiRepositoriesProducerPool.getLogName(logName, USStateHelper.EAST),
					getLogSize());
			manager.createIfNotExists(MultiRepositoriesProducerPool.getLogName(logName, USStateHelper.WEST),
					getLogSize());
		} else {
	...
	protected void startProducer(LogManager manager, List<List<String>> csvChunks, int nbThreads)
		...
		AccountMessageProducerFactoryMT factory = new AccountMessageProducerFactoryMT(repositoryName, csvChunks);
