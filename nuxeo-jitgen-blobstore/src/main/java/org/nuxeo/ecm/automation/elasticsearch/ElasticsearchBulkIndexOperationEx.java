 ...
 * @since 10.3
 */
@Operation(id = ElasticsearchBulkIndexOperationEx.ID, category = Constants.CAT_SERVICES, label = "Elasticsearch Indexing", since = "10.3", description = "Enable to index Nuxeo documents using the Bulk Service.", addToStudio = false)
public class ElasticsearchBulkIndexOperationEx {
    private static final Log log = LogFactory.getLog(ElasticsearchBulkIndexOperationEx.class);

    public static final String ID = "Elasticsearch.BulkIndexEx";
    ...
    public Blob run() throws IOException {
        checkAccess();
        esa.dropAndInitRepositoryIndex(getRepositoryName(), false);
    ...
    protected String submitBulkCommand(String nxql, boolean syncAlias) {
        ...
        return bulkService.submit(
                new BulkCommand.Builder(ACTION_NAME, nxql, username).param(INDEX_UPDATE_ALIAS_PARAM, syncAlias)
                                                                    .repository(getRepositoryName()).build());
    }
