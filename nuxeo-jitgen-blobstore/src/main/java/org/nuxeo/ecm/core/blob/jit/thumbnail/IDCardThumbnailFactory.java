...
public class IDCardThumbnailFactory implements ThumbnailFactory {

    private static final Log log = LogFactory.getLog(IDCardThumbnailFactory.class);
	...
	public Blob getThumbnail(DocumentModel doc, CoreSession session) {
	        ...
	        try {
	            ...
	            if (bh != null) {
	            	Blob source = bh.getBlob();
	            	if (source.getMimeType().equalsIgnoreCase("application/pdf")) {
	            		thumbnailBlob = computeThumb(source.getByteArray());
	            	} else {
	            		thumbnailBlob = source;
	            	}	            			
	            }
	        } catch (Exception e) {
